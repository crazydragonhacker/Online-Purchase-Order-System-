...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QLineEdit, QLabel, QPushButton, QMessageBox,
)

from . import db
from .config import asset_path, APP_ICON
...
class MainWindow(QWidget):
    ...
    def registation(self):
        ...
        street1 = self.txtStreet1.text().strip()
        street2 = self.txtStreet2.text().strip()
        city = self.txtCity.text().strip()
        state = self.txtState.text().strip()
        ...
        try:
            db.insert_customer(self.conn, fname, lname, email, street1, street2, city, state)
        except Exception as exc:
            QMessageBox.critical(self, "Database Error", f"Could not register customer:\n{exc}")
            return None

        QMessageBox.about(self, "Customer Registration", "Customer Registered Successfully")

        from .purchase_order_form import FrameSecond
        self.Winsecond = FrameSecond()
        self.Winsecond.setWindowTitle("Purchase Order Form")
        if APP_ICON.exists():
            self.Winsecond.setWindowIcon(QIcon(str(APP_ICON)))
